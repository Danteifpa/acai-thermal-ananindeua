...
import { Beaker, Info, Save, Loader2, Activity } from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-12 gap-8 max-w-7xl mx-auto">
      ...
      <div className="col-span-12 lg:col-span-4 space-y-6">
        <Card className="bg-slate-900 border-slate-800 p-8 space-y-8 shadow-2xl">
          <div className="space-y-2">
            <h2 className="text-2xl font-black text-white tracking-tight">Configuração</h2>
            <p className="text-slate-500 text-xs font-medium">Ajuste os parâmetros da amostra.</p>
          </div>
          
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">ID do Batedouro</Label>
              <input 
                type="text"
                value={nomeBatedouro}
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-end">
                <Label className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Volume (L)</Label>
                <span className="text-purple-400 font-black text-2xl font-mono">{volume}</span>
              </div>
              <Slider value={[volume]} onValueChange={(val) => setVolume(val[0])} max={50} min={1} step={1} className="py-2" />
            </div>

            <div className="space-y-3">
              <Label className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Material</Label>
              <div className="grid grid-cols-2 gap-2">
                <button 
                  onClick={() => setMaterial('Metal')}
                  className={`p-4 rounded-xl border text-xs font-bold uppercase transition-all ${material === 'Metal' ? 'bg-purple-600 border-purple-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-500'}`}
                >
                  Aço Inox
                </button>
                <button 
                  onClick={() => setMaterial('Plástico')}
                  className={`p-4 rounded-xl border text-xs font-bold uppercase transition-all ${material === 'Plástico' ? 'bg-purple-600 border-purple-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-500'}`}
                >
                  Polímero
                </button>
              </div>
          ...
          <div className="pt-4 space-y-4">
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button variant="outline" className="w-full border-slate-800 text-slate-400 hover:bg-slate-800 h-12 rounded-xl gap-2">
              ...
              <DialogContent className="bg-slate-900 border-slate-800 text-white">
                ...
                <div className="space-y-4 py-4 font-mono">
                  ...
                  <div className="flex justify-between p-3 bg-slate-950 rounded-lg border border-slate-800">
                    ...
                    <span className="text-purple-400">{material === 'Metal' ? 0.50 : 2.30} kJ/kg·K</span>
                  </div>
                </div>
              </DialogContent>
            </Dialog>
