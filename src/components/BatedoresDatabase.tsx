...
import { Loader2, Play, Upload, Database, Search, Filter } from 'lucide-react';
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const formattedData = rawData.map((row: any) => {
          ...
          const volume = parseFloat(String(row['Unidade (Litros/Lata)']).replace(',', '.'));
        ...
        showSuccess(`${formattedData.length} registros migrados para o Supabase.`);
        fetchBatedores();
      } catch (err: any) {
        showError("Erro na migração: " + err.message);
      } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 flex gap-3">
          ...
          <Select value={bairroFilter} onValueChange={setBairroFilter}>
            <SelectTrigger className="w-[200px] h-12 rounded-xl border-slate-200">
        ...
        <div className="flex gap-3">
          ...
          <Button asChild disabled={isProcessing} className="w-full bg-[#1E562F] hover:bg-[#164023] h-12 rounded-xl font-bold gap-2 cursor-pointer">
            <label htmlFor="csv-migrator">
              {isProcessing ? <Loader2 className="animate-spin" size={18} /> : <Upload size={18} />}
              Migrar CSV (132 Linhas)
            </label>
          </Button>
        </div>
      </div>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={b.id} className="border-slate-100 hover:bg-slate-50 transition-colors">
                ...
                <TableCell><Badge variant="outline" className="border-slate-200 text-slate-600">{b.material}</Badge></TableCell>
                <TableCell className="text-[#1E562F] font-mono font-bold">{b.volume_padrao}L</TableCell>
                <TableCell>
                  <Badge className={b.status_risco?.includes('🔴') ? "bg-red-50 text-red-600 border-red-100" : "bg-emerald-50 text-emerald-6000 border-emerald-100"}>
                    {b.status_risco}
                  </Badge>
                </TableCell>
