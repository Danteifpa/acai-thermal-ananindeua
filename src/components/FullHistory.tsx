...
const FullHistory = ({ records }: FullHistoryProps) => {
  const exportToPDF = (isCompliance = false) => {
    ...
    if (isCompliance) {
      ...
      doc.text('ANANINDEUA / PARÁ', 14, 29);
      
      doc.setFontSize(18);
      doc.setTextColor(0);
      doc.text('RELATÓRIO DE CONFORMIDADE TÉRMICA', 14, 40);
    } else {
      doc.setFontSize(20);
    ...
    doc.setFontSize(11);
    doc.setTextColor(100);
    doc.text(`Gerado em: ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, 14, isCompliance ? 52 : 30);
    ...
    if (isCompliance) {
      ...
      doc.text('Assinatura do Responsável Técnico (SEMAD)', 14, finalY + 35);
      doc.text('__________________________________________', 120, finalY + 30);
      doc.text('Vigilância Sanitária - Ananindeua', 120, finalY + 35);
    }

    const fileName = isCompliance ? 'relatorio-vigilancia-ananindeua' : 'relatorio-acai-thermal';
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-400 text-lg">Gestão de dados e exportação de relatórios técnicos para Ananindeua/PA.</p>
        </div>
